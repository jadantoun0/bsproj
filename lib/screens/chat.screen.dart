...
import 'package:deaf_connect/models/message.model.dart';
import 'package:deaf_connect/models/transcript.model.dart';
...
import 'package:deaf_connect/screens/shortcuts.screen.dart';
import 'package:deaf_connect/utils/colors.dart';
import 'package:deaf_connect/utils/date_utils.dart';
import 'package:deaf_connect/utils/navigation_utils.dart';
import 'package:deaf_connect/widgets/chat/chatbox.dart';
import 'package:deaf_connect/widgets/common/inkwell_with_opacity.dart';
import 'package:deaf_connect/widgets/dialogs/transcript_dialog.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    super.initState();
    // when the keyboard is opened, we scroll to the bottom
  ...
  Future _scrollListener() async {
    ...
    if (_scrollController.position.pixels ==
      ...
      if (mounted) {
        ...
        await transcriptProvider.loadMoreMessages(transcript!.transcriptId);
      }
      if (mounted) {
        setState(() {
          isLoadingMore = false;
                          ...
                          child: Scrollbar(
                            child: SingleChildScrollView(
                              ...
                              child: Padding(
                                ...
                                child: _buildMessagesList(transcriptProvider),
                              ),
                            ),
                          ),
                        ),
                        ...
                        AnimatedContainer(
                          ...
                          child: Column(
                            children: [
                              if (isShortcutVisible)
                                Expanded(
                                  child: Consumer<ShortcutsProvider>(
                                    builder:
                                        (context, shortcutsProvider, child) {
                                      return Padding(
                              ...
                              if (isShortcutVisible) const SizedBox(height: 2)
                            ],
                          ),
                        )
                      ],
  ...
  Widget _buildMessagesList(TranscriptProvider transcriptProvider) {
    ...
    Map<DateTime, List<Message>> groupedMessages =
        groupMessagesByDate(transcript!.messages);
